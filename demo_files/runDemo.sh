...
# ../fasturtle --bufferleft="{{" -buffer-right="}}" --input=tokenizedDemo.LR-buffer.xml --output=myoutput_multitokenjson_LR-buffer.xml --tokens=tokensDemo.json,tokensDemo2.json
../fasturtle --buffer="__" --input=tokenizedDemo.xml --output=myoutput_multitokenjson.xml --tokens=tokensDemo.json,tokensDemo2.json
# # ../fasturtle --bufferleft="{{" -buffer-right="}}" --input=tokenizedDemo.xml --output=myoutput_databag.xml --databag=_default
# # ../fasturtle --bufferleft="{{" -buffer-right="}}" --input=tokenizedDemo.xml --databag=fasturtle --databag-secret=dbsecret --output=results.xml
...
../fasturtle --buffer="__"  --input=tokenizedDemoQuoted.xml --output=myoutput_multitokenjson_assemblies_unquoted.xml --tokens=tokensDemo.json,tokensDemo2.json --unquoted
